...
def _attributes(instance: dict[str, Any]) -> dict[str, Any]:
...
def _exact_seed_instance(existing: dict[str, Any], pool: dict[str, Any]) -> dict[str, Any]:
    ...
    matches = [
        ...
        if instance.get("observation") == POOL_WAIT
        and instance.get("state") == "observed"
        and _attributes(instance).get("otel.trace_id") == request["trace_id"]
        and _attributes(instance).get("otel.span_id") == request["span_id"]
        and _attributes(instance).get("causcope.code_symbol") == request["code_symbol"]
        and _attributes(instance).get("causcope.runtime_resource") == pool_id
    ]
    if len(matches) != 1:
        raise RailsPoolEvidenceImportError(
...
def _identity(seed: dict[str, Any], pool: dict[str, Any], incident_id: str) -> str:
    ...
    if pool.get("system_id") != attributes.get("causcope.system_id"):
        raise RailsPoolEvidenceImportError("resource-pool evidence system_id does not match canonical seed evidence")
    if pool.get("revision", {}).get("value") != attributes.get("causcope.revision"):
...
def _source(
    *, pool: dict[str, Any], seed: dict[str, Any], target_resource: str,
    verification_id: str, baseline_revision: int, baseline_hypothesis: str | None,
    phase: str,
) -> dict[str, Any]:
    attributes = {
    ...
    return {"type": "experiment", "name": "causcope.rails.connection_pool_probe", "attributes": attributes}


def _instance(
    *, incident_id: str, pool: dict[str, Any], observation: str, state: str, phase: str,
    source: dict[str, Any], scope: dict[str, Any], target_resource: str,
    measurement: dict[str, Any], note: str, labels: dict[str, str] | None = None,
) -> dict[str, Any]:
    item_labels = {
...
def build_canonical_pool_evidence(
    pool: dict[str, Any], existing: dict[str, Any], snapshot: dict[str, Any]
) -> dict[str, Any]:
    ...
    target_resource = _identity(seed, pool, incident_id)
    ...
    if any(
        _attributes(instance).get("causcope.verification_id") == verification_id
        for instance in existing.get("instances", [])
    ):
    ...
    at_capacity = bool(assertions["application_pool_was_at_capacity"])
    instances.append(
        _instance(
            incident_id=incident_id, pool=pool, observation=POOL_UTILIZATION,
            ...
            labels={
                "mechanism_explains_request_delta": str(bool(assertions["checkout_wait_explains_request_delta"])).lower(),
                "baseline_hypothesis_rank": str(baseline_rank or 0),
            },
            note="Exact application pool occupancy before capacity release, bound to the canonical seed request and target.",
        )
    )

    if assertions["query_latency_stayed_near_baseline"]:
        current = float(pool["request"]["dependency_latency_ms"])
        baseline = float(pool["baseline"]["dependency_latency_ms"])
        instances.append(
            _instance(
                incident_id=incident_id, pool=pool, observation=QUERY_LATENCY,
                state="absent", phase="control", source=source("control"), scope=scope,
                target_resource=target_resource,
                measurement={"value": current, "baseline": baseline, "delta": current - baseline, "unit": "ms"},
                labels={
                    "independent_database_control": "reachable" if assertions["database_still_accepts_direct_connections"] else "not_established"
                },
                note="Elevated query latency was absent while the application pool was saturated.",
            )
        )
    ...
    for assertion, observation, field, note in recovery_specs:
        if not assertions[assertion]:
            continue
        current = float(pool["recovery"][field])
        baseline = float(pool["baseline"][field])
        instances.append(
            _instance(
                incident_id=incident_id, pool=pool, observation=observation,
    ...
    return {
        ...
        "description": "Canonical mechanism, control, and post-intervention recovery observations from the exact Rails D3.1 experiment.",
        "instances": instances,
    }
...
def import_pool_evidence(*, workspace: Path, pool_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "target_resource": _attributes(produced["instances"][0])["causcope.target_resource"],
        "projected_observations": [item["observation"] for item in produced["instances"]],
        "verification_id": _attributes(produced["instances"][0])["causcope.verification_id"],
    }


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Project one exact Rails D3.1 resource-pool experiment into canonical runtime evidence and atomically rerank the persisted Investigation."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(f"Investigation {result['incident_id']}: evidence revision {result['previous_evidence_revision']} -> {result['evidence_revision']}")
