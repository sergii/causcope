# RFC 0067: Recommendation information-gap ranking
...
That state is epistemically correct, but it is not yet operationally useful enough. A user should not have to inspect a JSON document and reverse-engineer what Causcope needs next.

RFC 0067 adds a derived projection:
...
## Relationship to RFC 0066 provider information-gain routing

RFC 0066 ranks already-eligible diagnostic providers by how much causal contrast their evidence semantics can resolve.

RFC 0067 sits one level above that concern:

```text
RFC 0067
  -> what information should Causcope acquire next?

if the answer is a read-only probe:
  -> existing probe/instrument routing
  -> RFC 0066 may help choose the best eligible provider
```

The two rankings are complementary and must not be collapsed into one score.

## Acquisition kinds

RFC 0067 distinguishes three information-acquisition kinds:
...
It does not execute the probe. Existing provider discovery, target-aware routing, provider information-gain routing, and probe-execution layers remain the execution authority.
...
RFC 0067 does not authorize state-changing execution.
...
RFC 0067 projects RFC 0065 recommendation states into next-action states.
...
RFC 0067 deterministically ranks:
...
RFC 0067 then ranks fresh problem evidence ahead of all architecture questions and points to the existing cataloged read-only probe:
...
RFC 0067 emits a `safe_experiment` action.
...
RFC 0067 still asks for measured before/after evidence.
...
RFC 0067 fails closed when RFC 0065 emits a missing-assumption identifier for which no information-gap semantics exist.
