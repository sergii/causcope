...
PROBE_OBSERVATIONS = {
    ...
    "probe.database.inspect_long_running_transactions": ["observation.database.long_running_transaction"],
    "probe.database.inspect_vacuum_health": [
        "observation.database.vacuum_pressure",
        "observation.database.autovacuum_disabled",
    ],
}
SUPPORTED_PROBES = set(PROBE_OBSERVATIONS)
...
def _blocking_chains(activity: list[dict[str, Any]]) -> list[list[int]]:
    edges = {
        int(row["pid"]): sorted({int(value) for value in (row.get("blocking_pids") or [])})
        for row in activity if row.get("blocking_pids")
    }
    chains: set[tuple[int, ...]] = set()

    def walk(path: tuple[int, ...]) -> None:
        blockers = edges.get(path[-1], [])
        if not blockers:
            chains.add(path)
            return
...
def collect_postgresql_health(database_url: str, *, long_transaction_seconds: int = DEFAULT_LONG_TRANSACTION_SECONDS) -> dict[str, Any]:
    if not database_url:
    ...
    with psycopg.connect(database_url, row_factory=dict_row) as connection:
        ...
        activity = connection.execute("""
            SELECT pid, state, backend_type, wait_event_type,
                   pg_blocking_pids(pid) AS blocking_pids,
                   CASE WHEN xact_start IS NULL THEN NULL
                        ELSE EXTRACT(EPOCH FROM (clock_timestamp() - xact_start)) END AS xact_age_seconds
              FROM pg_stat_activity
             WHERE datname = current_database() AND pid <> pg_backend_pid()
        """).fetchall()
        tables = connection.execute("""
            SELECT stats.schemaname, stats.relname, stats.n_dead_tup,
                   class.reltuples, class.reloptions
    ...
    for row in activity_rows:
        ...
        if age_seconds >= long_transaction_seconds:
            long_transactions.append({"pid": int(row["pid"]), "state": str(row.get("state") or "unknown"), "age_seconds": age_seconds})
    ...
    for raw in tables:
        ...
        table_max = global_max if table_max_value is None else (None if table_max_value == "-1" else float(table_max_value))
        if table_max is not None:
            trigger = min(trigger, table_max)
        dead_tuples = float(row.get("n_dead_tup") or 0.0)
        pressure_ratio = dead_tuples / trigger if trigger > 0 else (1.0 + dead_tuples if dead_tuples > 0 else 0.0)
        table_health.append({
            "schema": str(row["schemaname"]), "table": str(row["relname"]),
            "dead_tuples": dead_tuples, "estimated_tuples": reltuples,
            "vacuum_trigger": trigger, "pressure_ratio": pressure_ratio,
            "autovacuum_enabled": enabled, "trigger_exceeded": dead_tuples > trigger,
        })

    return {
        "collected_at": _utc_now(), "database": str(identity["database"]),
        ...
        "vacuum": {"autovacuum_enabled": global_autovacuum, "tables": table_health},
    }
...
class PostgresqlHealthCollector:
    ...
    def availability(self) -> tuple[bool, str | None]:
        if not os.environ.get(self.database_url_env):
    ...
    def __call__(self) -> dict[str, Any]:
        ...
        if not database_url:
            raise ValueError(f"required database URL environment variable is not set: {self.database_url_env}")
        return collect_postgresql_health(database_url, long_transaction_seconds=self.long_transaction_seconds)


class PostgresqlHealthAutonomousProvider:
    def __init__(self, *, concepts: dict[str, dict[str, Any]], incident_id: str, scope: dict[str, Any], target_resource: str, collector: HealthCollector, source_uri: str = "postgresql://health") -> None:
        if not incident_id or not target_resource:
            raise ValueError("PostgreSQL health provider requires incident_id and target_resource")
        ...
        for probe_id in SUPPORTED_PROBES:
            ...
            if not isinstance(probe, dict) or probe.get("kind") != "probe" or probe.get("risk") != "read_only":
                raise ValueError(f"PostgreSQL health provider requires canonical read-only probe: {probe_id}")
    ...
    def capability_projection(self) -> dict[str, Any]:
        ...
        for probe_id in sorted(SUPPORTED_PROBES):
            ...
            probes.append({
                ...
                "mapped_observations": sorted(PROBE_OBSERVATIONS[probe_id]),
            })
        return {
            "id": POSTGRESQL_HEALTH_PROVIDER_ID, "instrument": POSTGRESQL_HEALTH_INSTRUMENT,
            "transport": "postgresql_catalog", "scope_mode": "fixed_exact",
            "scope": copy.deepcopy(self.scope), "availability": self._availability(),
            "contract": {"name": "postgresql_catalog_health", "accepted_schema_versions": ["0.1"]},
            "evidence_semantics": {"complete_snapshot": True, "negative_result": "absent_evidence", "sql_text_collected": False, "causal_authority": False},
            "probes": probes,
        }
    ...
    def _validate_scope(self, scope: dict[str, Any] | None) -> dict[str, Any]:
        ...
        if scope_key(normalized) != scope_key(self.scope):
            raise ProbeInsufficientEvidence("PostgreSQL health provider scope does not match the selected diagnosis scope")
        return normalized

    def _instance(self, *, probe_id: str, observation: str, state: str, collected_at: str, database: str, measurement: dict[str, Any], labels: dict[str, str], scope: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "observation": observation, "state": state, "observed_at": collected_at, "confidence": "high",
            "source": {"type": "probe", "name": probe_id, "uri": self.source_uri, "attributes": {
                "provider": POSTGRESQL_HEALTH_PROVIDER_ID, "instrument": POSTGRESQL_HEALTH_INSTRUMENT,
                "database": database, "target_resource": self.target_resource,
            }},
            "scope": bound_scope, "measurement": measurement,
            "labels": {"probe": probe_id, "provider": POSTGRESQL_HEALTH_PROVIDER_ID, **labels},
    ...
    def execute(self, probe_id: str, target: str, scope: dict[str, Any] | None) -> dict[str, Any]:
        ...
        collected_at, database = str(snapshot["collected_at"]), str(snapshot["database"])
        instances = []
        if probe_id == "probe.database.inspect_lock_waits":
            ...
            for observation, unit in (("observation.database.blocking_chain", "chains"), ("observation.database.lock_wait_event", "blocked_sessions")):
                instances.append(self._instance(
                    probe_id=probe_id, observation=observation, state=state, collected_at=collected_at, database=database,
                    measurement={"value": len(chains), "unit": unit, "comparison": "present" if chains else "absent"},
                    labels=labels, scope=normalized_scope,
                ))
            ...
            instances.append(self._instance(
                probe_id=probe_id, observation="observation.database.long_running_transaction",
                state="observed" if transactions else "absent", collected_at=collected_at, database=database,
                measurement={"value": max_age, "baseline": threshold, "unit": "seconds", "comparison": "above_baseline" if transactions else "below_baseline"},
                labels={"transaction_count": str(len(transactions)), "threshold_seconds": str(threshold)}, scope=normalized_scope,
            ))
        else:
            ...
            instances.append(self._instance(
                probe_id=probe_id, observation="observation.database.vacuum_pressure",
                state="observed" if pressured else "absent", collected_at=collected_at, database=database,
                measurement={"value": max_ratio, "baseline": 1.0, "unit": "trigger_ratio", "comparison": "above_baseline" if pressured else "below_baseline"},
                labels={"pressured_table_count": str(len(pressured)), "table_count": str(len(tables))}, scope=normalized_scope,
            ))
            instances.append(self._instance(
                probe_id=probe_id, observation="observation.database.autovacuum_disabled",
                state="observed" if disabled else "absent", collected_at=collected_at, database=database,
                measurement={"value": len(disabled), "unit": "tables", "comparison": "present" if disabled else "absent"},
                labels={"disabled_table_count": str(len(disabled)), "global_autovacuum": "on" if vacuum.get("autovacuum_enabled") else "off"}, scope=normalized_scope,
            ))
        return {"schema_version": "0.1", "kind": "runtime_evidence", "incident_id": self.incident_id, "description": f"Direct PostgreSQL health evidence for {probe_id} on {self.target_resource}.", "instances": instances}
