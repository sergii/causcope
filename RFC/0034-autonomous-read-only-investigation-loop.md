# RFC 0034: Autonomous read-only investigation loop
...
cannot compare cohorts when the captured window contains only one cohort.
...
For example, the Shop lock-error probe does not treat an uncorrelated `database is locked` message as evidence for an arbitrary failing request. The testbed now carries request metadata into the SQLite operational-error log so the probe can correlate:
...
The first autonomous provider is a static Shop structured-log adapter.

It supports two canonical probes:
...
The probe compares failure rates for the same HTTP method and path across client platform/version cohorts.
...
into the selected failing request scope while preserving the working and failing cohort identities in labels.

The observation remains a discriminator, not causal proof.

### Database lock-error probe

The probe searches the captured diagnostic stream for explicit lock errors correlated to the selected request metadata.
...
This observation is a strong prediction of:

```text
hypothesis.database.lock_contention
```

when the application surfaces lock acquisition failure or timeout.
...
The structured-log adapter historically produced both direct request outcomes and higher-order derived findings in one pass.

That behavior remains the default for backward compatibility.
...
`autonomous-run.json` records for every attempted step:
...
This lets a human or agent answer not only "what does Causcope think?" but also "what did it inspect to change its mind?"

## Relationship to MCP probe sessions

This RFC does not replace the existing MCP begin/finish workflow.

The two paths serve different execution shapes:
...
RFC 0033 defines specialized deterministic analyzers as instruments that feed Causcope evidence.

Those adapters can later become autonomous probe providers when they expose a narrow read-only operation with trustworthy scope and deterministic output.
...
`scripts/test_autonomous_investigation.py` proves two convergence cases:
...
The Shop GitHub Actions workflow runs the same autonomous path against the real FastAPI + SQLite reference application for both fault scenarios.

The CI assertion reads final Causcope evidence and diagnosis. It never reads `oracle.json` to construct the diagnosis.
...
1. Add a versioned schema for autonomous-run audit records if a second consumer needs the report contract.
2. Expose eligible autonomous steps through MCP/HTTP without weakening the existing explicit probe-session safety path.
3. Add provider capability metadata so a provider can declare source requirements before execution where that declaration is deterministic.
4. Integrate the first external deterministic provider, likely PostgreSQL via the pgbot adapter described in RFC 0033.
