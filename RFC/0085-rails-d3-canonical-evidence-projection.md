# RFC 0085: Rails D3.1 canonical evidence projection
...
Those are important verification facts, but the current semantic graph does not yet define canonical observations for all of them. RFC 0085 does not invent generic observations merely to make the old X-Ray status fit the new path.
